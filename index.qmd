...
# Déterminer si suren_depot_taux est un vrai taux (‰ ménages) ou un nombre brut
suren_is_rate = (
    DATA_AVAILABLE
    and "suren_depot_taux" in df.columns
    and pd.to_numeric(df["suren_depot_taux"], errors="coerce").median() < 100
)
suren_unit_label = "Dépôts / 1 000 mén." if suren_is_rate else "nb brut"
suren_axis_label = "Taux de surendettement (‰ ménages)" if suren_is_rate else "Dépôts de surendettement (nb)"
```
...
if EDA_AVAILABLE:
    plot_vars = [
        ("suren_depot_taux",  "Taux de surendettement",     suren_unit_label,       "BdF 2021"),
...
if DATA_AVAILABLE and len(df) > 0:
    ...
    if available_trend and df["annee"].nunique() >= 2:
        ...
        ax2.set_ylabel(suren_axis_label, fontsize=10, color="#d62728")
